...
public class Q1TwoSum {
    ...
    /*
    Map
    这个方法用一个Map来存储数组当中的key和value的，这里为了方便，把数组内的value设置为map的key，而index为value。
    这个时候，如果这个map里面有target - nums[i]对应的value，就代表这俩map中的value是答案，然后每一次循环都把这个循环的东西放进去。时间复杂度为O(n)。
    同时，这个方法也满足不重复用数的规则。
     */
